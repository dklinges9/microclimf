...
.checkinputs <- function(weather, rainfall, vegp, soilc, dtm, merid = 0, dst = 0, daily = FALSE) {
  ...
  check.vals(weather$swrad,0,1500,"shortwave radiation","W / m^2")
  check.vals(weather$difrad,0,1500,"diffuse radiation","W / m^2")
  ...
  # Wind direction
  mn<-min(weather$winddir,na.rm=T)
  mx<-max(weather$winddir,na.rm=T)
  if (mn<0 | mx>360) {
